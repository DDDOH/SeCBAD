...
class AntVelNon(AntEnv):
    """ant, 设定一个向右的velocity，reward 为 |target velocity - agent velocity|"""
    ...
    def sample_new_task(self):
        ...
        target_vel_base = np.random.uniform(0, 3)
        return target_vel_base
    ...
    def step(self, action):
        ...
        self.goal_velocity = self.goal_velocity_base + np.random.normal(0, 0.2)

        torso_xyz_before = np.array(self.get_body_com("torso"))

        self.do_simulation(action, self.frame_skip)
        torso_xyz_after = np.array(self.get_body_com("torso"))
        agent_velocity = (torso_xyz_after - torso_xyz_before)[:2] / self.dt

        velocity_cost = - np.linalg.norm(
            agent_velocity - np.array([self.goal_velocity, 0]))

        ctrl_cost = .05 * np.square(action).sum()
        contact_cost = 0.5 * 1e-3 * np.sum(
            np.square(np.clip(self.data.cfrc_ext, -1, 1)))
        reward = 1 * velocity_cost - ctrl_cost - \
            contact_cost
