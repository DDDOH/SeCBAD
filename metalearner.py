...
class MetaLearner:
    ...
    def train(self):
        ...
        for self.iter_idx in progressbar.progressbar(range(self.num_updates)):
    ...
    def log(self, run_stats, train_stats, start_time):
        ...
        if ((self.iter_idx + 1) % self.args.log_interval == 0) and (train_stats is not None):
            ...
            self.logger.add(
                'encoder/latent_mean', torch.cat(self.policy_storage.latent_mean).mean(), self.iter_idx)
            self.logger.add('encoder/latent_logvar',
                            torch.cat(self.policy_storage.latent_logvar).mean(), self.iter_idx)
            ...
            for [model, name] in [
                ...
                if model is not None:
                    ...
                    self.logger.add('weights/{}'.format(name),
