...
class Policy(nn.Module):
    def __init__(self,
        ...
        if policy_initialisation == 'normc':
            def init_(m): return init(m, init_normc_, lambda x: nn.init.constant_(
        ...
        if self.pass_state_to_policy and self.use_state_encoder:
            self.state_encoder = utl.FeatureExtractor(
                dim_state, self.args.policy_state_embedding_dim, self.activation_function)
        ...
        if self.pass_latent_to_policy and self.use_latent_encoder:
            self.latent_encoder = utl.FeatureExtractor(
                dim_latent, self.args.policy_latent_embedding_dim, self.activation_function)
        ...
        if action_space.__class__.__name__ == "Discrete":
            ...
            self.dist = DiagGaussian(
                hidden_layers[-1], num_outputs, init_std, self.args.norm_actions_pre_sampling)
        else:
        ...
        if self.pass_state_to_policy:
            if self.norm_state:
                state = (state - self.state_rms.mean) / \
                    torch.sqrt(self.state_rms.var + 1e-8)
        ...
        if self.pass_latent_to_policy:
            if self.norm_latent:
                latent = (latent - self.latent_rms.mean) / \
                    torch.sqrt(self.latent_rms.var + 1e-8)
    ...
    def update_rms(self, args, policy_storage):
        ...
        if self.pass_latent_to_policy and self.norm_latent:
            latent = utl.get_latent_for_policy(args,
                                               torch.cat(
                                                   policy_storage.latent_samples[:-1]),
                                               torch.cat(
