...
class MetaLearner:
    ...
    def train(self):
        ...
        # reset environments
        prev_state, belief, task, _ = utl.reset_env(self.envs, self.args)
            ...
            for step in range(self.args.policy_num_steps):
                ...
                self.policy_storage.insert(
                    ...
                    ctrl_reward=torch.FloatTensor(
                        [info['ctrl_reward'] for info in infos]).unsqueeze(0).T.to(device) if 'ctrl_reward' in infos[0].keys() else None,
                    vel_reward=torch.FloatTensor(
                        [info['vel_cost'] for info in infos]).unsqueeze(0).T.to(device) if 'vel_cost' in infos[0].keys() else None
                )
